# 从零构建 12-factor agent 模板

> 中文翻译：由 0bipinnata0 翻译 / 改编自原项目对应英文页面。内容与图片遵循 CC BY-SA 4.0；代码遵循 Apache 2.0。
> 原文：`workshops/2025-05-17/walkthrough.md`，基准提交：`ca5958c`

本教程会从一个空的 TS 仓库开始，一步步构建出一个 12-factor agent。你将创建一个遵循 12-factor 方法论的 TypeScript agent。
...
随着你添加更多测试，可以关闭日志，让输出保持干净。
当你针对特定测试迭代时，可能会想把日志打开。
...
添加一个 Express 服务器，通过 HTTP 暴露 agent。
...
* 从 `/thread` 端点返回 thread ID 和 response URL
* 实现 `GET /thread/:id`
...
你应该会看到：最后一次工具调用现在变成 `"intent":"divide","a":3,"b":5`。
...
到这里就结束了。下一章我们会构建一个完全由 email 驱动的工作流 agent，它会使用 webhooks 处理人工批准。


## 第 XX 章：HumanLayer Webhook 集成

前几节使用的是 humanlayer SDK 的“同步模式”。这意味着每次等待人工批准时，我们都会停在循环里不断轮询，直到收到人类响应。

这显然不理想，尤其是对生产工作负载来说。
所以本节会实现 [factor 6：用简单 API 启动 / 暂停 / 恢复](../../content/zh-CN/factor-06-launch-pause-resume.md)：更新服务器，让它在联系人类后结束处理，并使用 webhooks 接收结果。
...
更新服务器，让它能够处理 request_clarification 响应。

- 移除旧的 `/response` 端点和 types
- 更新 `/thread` 端点，让处理异步运行并立即返回
- 请求人类响应时发送 `state.threadId`
- 添加 `handleHumanResponse` 函数处理人类响应
- 添加 `/webhook` 端点处理 webhook 响应
...
服务器运行后，向 `/thread` 端点发送一个 payload。


- TODO：执行 response 步骤

- TODO：现在处理 divide 的 approvals

- TODO：现在也处理 done_for_now
